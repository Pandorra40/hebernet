    ...
    <aside class="side">
      <div class="brand">
        ...
        <div>
          ...
          <div class="role">Espace client</div>
        </div>
      </div>
      <nav>
        <RouterLink to="/client/sites" :class="{ on: route.path.startsWith('/client/sites') }">
          Mes sites
        </RouterLink>
      </nav>
      <div class="foot">
        <div class="muted small">{{ user?.display_name || user?.email }}</div>
...
.main { padding: 1.75rem 2rem; max-width: 960px; }
</style>
